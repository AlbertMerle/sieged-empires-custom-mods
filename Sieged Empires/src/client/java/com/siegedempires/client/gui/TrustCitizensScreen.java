...
public class TrustCitizensScreen extends Screen {
	...
	public TrustCitizensScreen(Screen parent) {
		super(Component.translatable("gui.siegedempires.trust_citizens_title"));
		this.parent = parent;
	}
	...
	private void rebuildMemberList() {
		...
		for (ManageTownData.MemberInfo member : data.members) {
			...
			boolean trusted = "Trusted Citizen".equals(member.role);
			Component label = trusted
			...
			Button button = Button.builder(label, btn -> {
				if (!trusted) {
					minecraft.gui.setScreen(new ConfirmTrustCitizenScreen(this, member));
				}
