...
public class EvictPlayerScreen extends Screen {
	...
	public EvictPlayerScreen(Screen parent) {
		super(Component.translatable("gui.siegedempires.evict_player_title"));
		this.parent = parent;
	}
	...
	private void rebuildMemberList() {
		...
		for (ManageTownData.MemberInfo member : data.members) {
			...
			Button button = Button.builder(Component.literal(member.name),
					btn -> minecraft.gui.setScreen(new ConfirmEvictPlayerScreen(this, member)))
